(function () {
  ...
  var MediaPopin = /*#__PURE__*/function (_PopinDefault) {
    _inherits(MediaPopin, _PopinDefault);

    var _super = _createSuper(MediaPopin);

    function MediaPopin() {
      var _this;

      _classCallCheck(this, MediaPopin);

      _this = _super.call(this);
      _this.specificClasses = ["js-media-popin-open"];
      _this.popinSelector = ".PopinMedia";
      _this.closePopinButtonTitle = Drupal.t("Fermer la modale Media");
      _this.popinTriggerSelector = ".Media-popin-trigger";
      return _this;
    }
    /**
    * Open modal when click on trigger
    */
    ...
    _createClass(MediaPopin, [{
      ...
      value: function triggerClick(ev) {
        ...
        this.addTabIndex(popin);
        popin.classList.add("open");
        var mediaId = ev.currentTarget.getAttribute("data-popin-media-id");
        var path = '/ajax/m_enhance/display/'; //let bundle = $trigger.data('popin-media-bundle');
        //if(bundle === 'video') {}

        Drupal.ajax({
          "url": path + mediaId
        }).execute().done(function (commands, statusString, ajaxObject) {
          var newClose = _this2.createCloseButton();
...
//# sourceMappingURL=mediaPopin.js.map
